...
<head>
  ...
  <title>PHP+DB PHPでデータを挿入しよう（INSERT）</title>
...
<body>
  ...
  <form action="update.php?id=<?= $_GET['id'] ?>" method="post">
    <div>
      ...
      <input type="email" id="user_email" name="user_email" value="<?= $user['email'] ?>" maxlength="60" required>

      <label for="user_age">年齢</label>
      <input type="number" id="user_age" name="user_age" value="<?= $user['age'] ?>" maxlength="60" required>

      <label for="user_address">住所</label>
      <input type="text" id="user_address" name="user_address" value="<?= $user['address'] ?>" maxlength="60" required>
    </div>
